...
`注意：let是只作用于当前的区块，不存在往下层延伸的特点，即`
      `但是var是存在这种特点的，即存在往内层延伸的特点`
...
_顶层对象的属性与全局变量挂钩，被认为是 JavaScript 语言最大的设计败笔之一。_

1. 这样的设计带来了几个很大的问题，首先是没法在编译时就报出变量未声明的错误，只有运行时才能知道（因为全局变量可能是顶层对象的属性创造的，而属性的创造是动态的）；
